...
function openScoreConfig() {
  ...
  const rows = Object.entries(cfg).map(([type, c]) => `
    <tr>
      <td><span style="display:inline-flex;align-items:center;gap:7px"><span style="width:9px;height:9px;border-radius:2px;background:${c.color}"></span>${c.label}</span></td>
      <td style="text-align:center"><input class="input sc-pt" data-type="${type}" type="number" value="${c.points}" style="width:64px;height:28px;text-align:center"></td>
      <td style="text-align:center">${c.perTime ? `<input class="input sc-cap" data-type="${type}" type="number" value="${c.cap}" style="width:64px;height:28px;text-align:center">` : '<span class="cell-mute">固定</span>'}</td>
      <td style="text-align:center"><span class="badge ${c.perTime?'b-info':'b-neutral'}">${c.perTime?'回数×点':'発生で固定'}</span></td>
    </tr>
  `).join('');

  openModal({
    title: '配点設定',
    subtitle: '行動イベントごとの配点ルール（変更すると即再計算）',
    size: 'lg',
    icon: `<div style="width:36px;height:36px;border-radius:9px;background:var(--accent-bg);color:var(--accent-hi);display:grid;place-items:center">${Icon('settings',18)}</div>`,
    body: `
      <table class="tbl" style="border:1px solid var(--border);border-radius:8px;overflow:hidden;margin-bottom:18px">
        <thead><tr><th>行動イベント</th><th style="text-align:center">配点</th><th style="text-align:center">上限</th><th style="text-align:center">計算方式</th></tr></thead>
        <tbody>${rows}</tbody>
      </table>

      <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:14px">
        <div>
          <div style="font-size:11px;color:var(--text-mute);letter-spacing:.05em;text-transform:uppercase;font-weight:600;margin-bottom:8px">合計スコア上限</div>
          <input class="input" id="sc-total" type="number" value="${SCORING.totalCap}" style="width:100%">
        </div>
        <div>
          <div style="font-size:11px;color:var(--text-mute);letter-spacing:.05em;text-transform:uppercase;font-weight:600;margin-bottom:8px">時間窓（ヶ月）</div>
          <input class="input" id="sc-window" type="number" value="${window.LEAD_WINDOW||2}" style="width:100%">
        </div>
        <div>
          <div style="font-size:11px;color:var(--text-mute);letter-spacing:.05em;text-transform:uppercase;font-weight:600;margin-bottom:8px">足切り（ヶ月）</div>
          <input class="input" id="sc-cutoff" type="number" value="${SCORING.cutoffMonths}" style="width:100%">
        </div>
      </div>
      <div style="font-size:11px;color:var(--text-mute);margin-top:6px">足切り：直近この期間に有効行動がないリードはスコア対象外になります。</div>

      <div style="margin-top:16px">
        <div style="font-size:11px;color:var(--text-mute);letter-spacing:.05em;text-transform:uppercase;font-weight:600;margin-bottom:8px">スコア帯しきい値</div>
        <div style="display:flex;gap:14px;align-items:center;font-size:12.5px">
          <span>ホット ≥ <input class="input" id="sc-hot" type="number" value="${SCORING.bands.hot}" style="width:64px;height:28px;text-align:center"></span>
          <span>ウォーム ≥ <input class="input" id="sc-warm" type="number" value="${SCORING.bands.warm}" style="width:64px;height:28px;text-align:center"></span>
          <span style="color:var(--text-mute)">それ未満はコールド</span>
        </div>
      </div>
    `,
    footer: `
      <button class="btn ghost" onclick="closeModal()">キャンセル</button>
      <button class="btn primary" onclick="saveScoreConfig()">保存して再計算</button>
    `,
  });
}

function saveScoreConfig() {
  document.querySelectorAll('.sc-pt').forEach(i => { SCORING.config[i.dataset.type].points = Number(i.value) || 0; });
  document.querySelectorAll('.sc-cap').forEach(i => { SCORING.config[i.dataset.type].cap = Number(i.value) || 0; });
  SCORING.totalCap = Number(document.getElementById('sc-total').value) || 100;
  SCORING.bands.hot = Number(document.getElementById('sc-hot').value) || 80;
  SCORING.bands.warm = Number(document.getElementById('sc-warm').value) || 40;
  window.LEAD_WINDOW = Number(document.getElementById('sc-window').value) || 2;
  SCORING.cutoffMonths = Number(document.getElementById('sc-cutoff').value) || 3;
  const winSel = document.getElementById('lead-window');
  if (winSel) winSel.value = String(window.LEAD_WINDOW);
  closeModal();
  buildLeadProductFilter();
  renderLeadMatrix();
  renderLeads();
  renderReconcile();
  toast('配点ルールを更新 → 全リードを再計算しました');
}

window.openScoreConfig = openScoreConfig;
